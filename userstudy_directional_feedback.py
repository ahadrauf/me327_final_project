...
if __name__ == '__main__':
    now = datetime.now()
    name_clarifier = "_userstudy_directional_feedback_user{}".format(user_number)
    ...
    while test_num < num_trials:
        ...
        if len(tags) > 0:
            inside, vector_to_center = update_ring(rings, tags[0].pose_t, tags[0].pose_R, ring_pts,
                                                   active_curve_midpoints, camera2World, ring_radius)
    ...
    if save_data:
        ...
        np.save("data/" + timestamp + ".npy",
                [data, test_direction_values, actual_direction_values, test_times, return_times, final_vectors_to_center,
